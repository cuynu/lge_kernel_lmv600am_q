...
struct cas_monitor_info cas_info;
unsigned int sysctl_sched_cas_enable = 0;
char sysctl_sched_cas_info[CAS_INFO_LENGTH];
...
static inline int cas_min_freq(void) {
	struct root_domain *rd = cpu_rq(smp_processor_id())->rd;

	if (rd == NULL || rd->min_cap_orig_cpu < 0)
		return 0;

	return schedutil_min_freq(rd->min_cap_orig_cpu);
}

static void cas_clear_info(int index) {
	cas_info.index = index;
	cas_info.isol_change = 0;
	cas_info.capacity = 0;
	cas_info.utilization = 0;
	cas_info.total_demand = 0;
	cas_info.big_task_demand = 0;
	cas_info.num_migration = 0;
}

#define CAS_INFO_COUNT 4
#define DEMAND_SCALE 19532

static void cas_update_info(u64 total_demand, u64 big_demand, int prev_cpu) {
	cpumask_t cpumask;
	int min_freq = 0;
	int i = 0;
	...
	if (cas_info.index <= CAS_INFO_COUNT) {
		if (cas_info.index == 0) {
			cas_info.prev_cpu = prev_cpu;
			cpumask_copy(&cas_info.prev_isolated_mask, cpu_isolated_mask);
			cas_info.index++;
			return;
		}

		cas_info.total_demand += total_demand / DEMAND_SCALE;
		cas_info.big_task_demand += big_demand / DEMAND_SCALE;
		for (i = 0; i < 8; i++) {
			if (cpu_isolated(i))
				continue;
			else {
				cas_info.capacity += capacity_curr_of(i);
				cas_info.utilization += cpu_util(i);
			}
		}

		if (!same_cluster(cas_info.prev_cpu, prev_cpu))
			cas_info.num_migration += 1;

		cpumask_andnot(&cpumask, &cas_info.prev_isolated_mask,
			cpu_isolated_mask);
		if (!cpumask_empty(&cpumask))
			cas_info.isol_change = 1;

		cas_info.prev_cpu = prev_cpu;
		cpumask_copy(&cas_info.prev_isolated_mask, cpu_isolated_mask);
		cas_info.index++;
	} else {
		min_freq = cas_min_freq();

		cas_info.total_demand /= CAS_INFO_COUNT;
		cas_info.big_task_demand /= CAS_INFO_COUNT;
		cas_info.capacity /= CAS_INFO_COUNT;
		cas_info.utilization /= CAS_INFO_COUNT;

		snprintf(sysctl_sched_cas_info, CAS_INFO_LENGTH, "%llu %llu %lu %lu %d %d %d",
				cas_info.total_demand,
				cas_info.big_task_demand, cas_info.capacity,
				cas_info.utilization, min_freq,
				cas_info.isol_change, cas_info.num_migration);
...
static void _set_preferred_cluster(struct related_thread_group *grp)
{
	...
	list_for_each_entry(p, &grp->tasks, grp_list) {
		...
		if (p->ravg.mark_start < wallclock -
			...
			continue;
#ifdef CONFIG_SCHED_CAS
		if( p->ravg.coloc_demand > big_demand) {
			big_demand = p->ravg.coloc_demand;
			prev_cpu = task_cpu(p);
		}
#endif /* CONFIG_SCHED_CAS */
...
#ifdef CONFIG_SCHED_CAS
	if (sysctl_sched_cas_enable == 1)
		cas_update_info(combined_demand, big_demand, prev_cpu);
#endif /* CONFIG_SCHED_CAS */
