...
#if defined(CONFIG_SCHED_WALT) && defined(CONFIG_SCHED_CAS)
unsigned int schedutil_freq_min(int cpu);
...
#if defined(CONFIG_SCHED_WALT) && defined(CONFIG_SCHED_CAS)
static inline unsigned int schedutil_min_freq(int cpu)
{
	return schedutil_freq_min(cpu);
}
...
struct cas_monitor_info {
	unsigned int index;
	unsigned int prev_cpu;
	unsigned int isol_change;
	unsigned int num_migration;
	unsigned long capacity;
	unsigned long utilization;
	cpumask_t prev_isolated_mask;
	u64 total_demand;
	u64 big_task_demand;
