...
static unsigned long acc_total_cas, acc_relative_busy_cas;
...
#ifdef CONFIG_SCHED_CAS
static ssize_t gpu_load_cas_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	unsigned long sysfs_busy_perc = 0;
	...
	spin_lock(&sample_lock);
	if (acc_total_cas)
		sysfs_busy_perc = (acc_relative_busy_cas * 100) / acc_total_cas;

	/* Reset the parameters */
	acc_total_cas = 0;
	acc_relative_busy_cas = 0;
	spin_unlock(&sample_lock);
	return snprintf(buf, PAGE_SIZE, "%lu\n", sysfs_busy_perc);
}
#endif /* CONFIG_SCHED_CAS */
...
#ifdef CONFIG_SCHED_CAS
static DEVICE_ATTR_RO(gpu_load_cas);
#endif /* CONFIG_SCHED_CAS */
...
#ifdef CONFIG_SCHED_CAS
	acc_total_cas += stats->total_time;
	acc_relative_busy_cas += ((u64)stats->busy_time *
