...
static int smb1390_get_prop_charging_enabled(struct smb1390 *chip)
{
	...
	if (unified_nodes_show("support_fastpl", buff)
		...
		smb1390_dbg(chip, PR_LGE, "get charging enable %d (0x%x, 0x%x, soc : %d, try : %d)\n",
			ret, status, status2, capa.intval, count);
		while (get_effective_result(chip->disable_votable)) {
			client = (char*) get_effective_client(chip->disable_votable);
			if (client != NULL)
				vote(chip->disable_votable, client, false, 0);
			pr_info("FASTPL: Clearing CP_DISABLE voter %s for test purpose\n", client);
		}
	} else {
		count = 0;
	}
