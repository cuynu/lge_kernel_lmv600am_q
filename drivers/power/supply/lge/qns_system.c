...
static bool lge_has_veneer_psy(void)
{
	if (data.veneer_psy == NULL) {
		data.veneer_psy = power_supply_get_by_name("veneer");
		if (data.veneer_psy == NULL) {
			pr_qns(PR_ERROR, "ERROR: unable to get veneer psy\n");
			return false;
		}
	}
	return true;
...
static int qns_get_scvt(int *soc, int *c, int *v, int *tx10)
{
	...
	*/
	...
	if (!qns_has_psy())
		retVal = QNS_ERROR;
	if (!lge_has_veneer_psy())
		retVal = QNS_ERROR;

	if (data.battery_psy && data.veneer_psy) {
		if (c != NULL) {
			if (power_supply_get_property(data.battery_psy,
