...
static void wa_comp_pps_pwr_dwork_func(struct work_struct *work)
{
	...
	if (!cp_enable ||
		...
		pr_info("retry reason -> CP STATUS, en=%d, sts1=0x%x, sts2=0x%x\n",
				cp_enable, cp_status1, cp_status2);
...
static void wa_bad_operation_pps_ta_work_func(struct work_struct *work)
{
	...
	struct smb_charger *chg = ext_chg->chg;
	union power_supply_propval val = {0, };
	int vusb_mv = 0, fcc_ma = 0, fcc_main_ma = 0;

	if (!chg->usb_psy)
		chg->usb_psy = power_supply_get_by_name("usb");

	if (!chg->usb_psy || !chg->fcc_votable || !chg->fcc_main_votable) {
		schedule_delayed_work(
...
void wa_helper_init(struct smb_charger *chg)
{
	...
	ext_chg->wa_typec_state_change_nb.notifier_call = wa_source_change_nb;
