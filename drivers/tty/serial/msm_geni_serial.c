...
static void msm_geni_serial_prep_rx_dma(void __iomem *base, dma_addr_t *rx_dma)
{
	...
	/*
	 * make sure prior operations are done before writing len,
	 * which starts dma operation.
	 */
	mb();
	geni_write_reg(DMA_RX_BUF_SIZE, base, SE_DMA_RX_LEN);
...
static void start_rx_sequencer(struct uart_port *uport)
{
	...
	if (geni_status & S_GENI_CMD_ACTIVE) {
		if (port->xfer_mode == SE_DMA && !port->rx_dma) {
			...
			msm_geni_serial_prep_rx_dma(uport->membase,
								&port->rx_dma);
		}
	...
	if (port->xfer_mode == FIFO_MODE) {
		...
		msm_geni_serial_prep_rx_dma(uport->membase, &port->rx_dma);
	}
	/*
...
exit_handle_dma_rx:
	...
	length = geni_read_reg(uport->membase, SE_DMA_RX_LEN);
	IPC_LOG_MSG(msm_port->ipc_log_rx,
			"%s:DMA Rx prior map base:0x%lx, dma_rx_l:0x%lx dma_rx_h:0x%lx length:0x%lx\n",
			__func__,  uport->membase, dma_rx_l, dma_rx_h, length);

	/* memsetting the RX_BUF to ZERO for debug purpose */
	memset(msm_port->rx_buf, 0, DMA_RX_BUF_SIZE);

	msm_geni_serial_prep_rx_dma(uport->membase, &msm_port->rx_dma);

	dma_rx_l = geni_read_reg(uport->membase, SE_DMA_RX_PTR_L);
	dma_rx_h = geni_read_reg(uport->membase, SE_DMA_RX_PTR_H);
	length = geni_read_reg(uport->membase, SE_DMA_RX_LEN);
	IPC_LOG_MSG(msm_port->ipc_log_rx,
			"%s:DMA Rx after map base:0x%lx, rx_dma:0x%lx dma_rx_l:0x%lx dma_rx_h:0x%lx length:0x%lx\n",
			__func__,  uport->membase, msm_port->rx_dma,
			dma_rx_l, dma_rx_h, length);
	if (ret)
...
static irqreturn_t msm_geni_serial_isr(int isr, void *dev)
{
	...
	IPC_LOG_MSG(msm_port->ipc_log_misc,
		"%s dma_tx_irq = 0x%x dma_rx_irq = 0x%x\n", __func__,
